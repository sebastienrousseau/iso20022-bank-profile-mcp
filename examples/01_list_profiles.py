...
server can lint against. Call this first to discover the ``profile_id``
values the other tools accept. Fully local; no sub-server involved.
...
from iso20022_bank_profile_mcp.server import list_profiles
...
def main() -> None:
    ...
    for profile in profiles:
        print(
            f"  {profile['profile_id']:<14}  "
            f"{profile['rule_count']} rule(s)  "
