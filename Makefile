...
lint: ## Run linters (ruff + black check)
	$(POETRY) run ruff check iso20022_bank_profile_mcp/ tests/
	$(POETRY) run black --check iso20022_bank_profile_mcp/ tests/

format: ## Auto-format code (ruff fix + black)
	$(POETRY) run ruff check --fix iso20022_bank_profile_mcp/ tests/
	$(POETRY) run black iso20022_bank_profile_mcp/ tests/

type-check: ## Run mypy type checking
	$(POETRY) run mypy iso20022_bank_profile_mcp/

security: ## Run security scan (bandit)
	$(POETRY) run bandit -r iso20022_bank_profile_mcp/ -c pyproject.toml 2>/dev/null || \
		$(POETRY) run bandit -r iso20022_bank_profile_mcp/ -ll
