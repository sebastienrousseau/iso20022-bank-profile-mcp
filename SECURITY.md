...
The iso20022-bank-profile-mcp maintainers take the security of this project
seriously. This document explains which versions receive security updates and
how to report a vulnerability responsibly.

iso20022-bank-profile-mcp is the bank clearing-profile Model Context Protocol
(MCP) server of the **ISO 20022 MCP Suite** — a fully local, closed-world
server (no network surface, no sub-servers) that manages, validates, and serves
the bank-specific clearing profiles / rule packs that sit beyond structural XSD
validation. It is a sibling of `iso20022-readiness-suite-mcp`, which can consume
the profiles this server serves, ahead of the November 2026 ISO 20022
milestones.
...
- **GitHub Security Advisories** (preferred): open a private report via the
  repository's
  [Security tab → "Report a vulnerability"](https://github.com/sebastienrousseau/iso20022-bank-profile-mcp/security/advisories/new).
...
- The `iso20022-bank-profile-mcp` MCP server as published in this
  repository, including the FastMCP tools it exposes over stdio
  (`list_profiles`, `get_profile`, `lint_payload`,
  `validate_profile_definition`) and the error envelopes it returns.
- Handling of agent-supplied tool arguments and the payloads returned to
  agents, including error envelopes. Inbound payloads and profile definitions
  are accepted as raw string content, never as server filesystem paths.
...
- Parsing and validation of bank-supplied profile / rule-pack definitions
  (`ProfileEngine.validate_definition`), including the rule mini-language and
  the assertion verbs it accepts.
- Input validation for clearing-profile identifiers and the linting surface.

The following are generally out of scope:

- Vulnerabilities in the foundational suite servers (iso20022-mcp,
  camt053-mcp, pain001-mcp, reconcile-mcp, bankstatementparser-mcp,
  structured-address-fix-mcp) or in `iso20022-readiness-suite-mcp`; please
  report those against their respective repositories.
...
Thank you for helping keep iso20022-bank-profile-mcp and its users safe.
...
| **PW.1** Design software to mitigate security risks | XML payloads are parsed with `defusedxml` only (no XXE / billion-laughs); the server returns serialised `{"error": ...}` payloads rather than raising into client transports; every tool is closed-world with no network surface. |
...
| **PW.8** Test executable code | pytest on Python 3.10–3.13 at 100% line + branch coverage; the profile engine and the rule mini-language are exercised directly against the bundled and fixture profiles. |
...
- **Fuzzing** — no continuous fuzzing harness ships in v0.0.1. The
  ...
  defusedxml-backed; a fuzzing target may be added later. Accepted for now.
