...
# `iso20022-bank-profile-mcp` governance

This document describes how `iso20022-bank-profile-mcp` is run, how
...
covers the profile-server-specific bits.

## Mission and scope

`iso20022-bank-profile-mcp` is the Model Context Protocol (MCP) server that
manages, validates, and serves the bank-specific ISO 20022 clearing profiles /
rule packs — the market-practice rules that sit beyond structural XSD
validation. It is a fully local, closed-world server (no network surface, no
sub-servers) and a sibling of `iso20022-readiness-suite-mcp`, which can consume
the profiles this server serves. Changes are weighed against the same criterion
as the rest of the suite: **correctness, security, and clarity over feature
breadth**.

A change is in-scope if it adds or hardens a clearing profile, the rule
mini-language, the profile/rule-pack validator, or the payload linter, or
improves the agent-driven workflow shape. A change is out-of-scope if it
duplicates logic that belongs in a foundational sub-server, opens a network
surface, or ships features that depend on a particular client (e.g.
...
- Larger changes (new tool surface, new transport, new profile, dependency
