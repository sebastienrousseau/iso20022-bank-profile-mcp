...
(`--transport http`, `http/transport.py`: OAuth 2.1 resource server or a
static dev-mode bearer token, `X-MCP-Tenant` scoping) for shared
...
Option 3. `iso20022-bank-profile-mcp` runs stdio;
`iso20022-bank-profile-mcp --transport streamable-http` listens on
...
`iso20022-bank-profile-mcp --transport sse` serves the older HTTP+SSE
...
deployment sits behind a gateway the operator trusts. The authenticated
`--transport http` with `--bind` stays as it was, an additional choice
...
every transport. The stdio and `http` tests stay as they were. Premium
entitlement over the two new transports comes only from the
`ISO20022_BANK_PROFILE_ENTITLEMENTS` allowlist, as it does over stdio:
there is no bearer token to carry an OAuth scope.
